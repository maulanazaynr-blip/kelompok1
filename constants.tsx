
import { Engineer } from './types';

export const ENGINEERS: Engineer[] = [
  {
    id: '1',
    name: 'Zayn',
    title: 'Geodetic Engineer',
    specialty: 'Geodetic Engineering',
    externalUrl: 'https://zayn-one.vercel.app/',
    bio: 'A dedicated professional with extensive experience in land surveying and mapping. Specialized in high-precision cartography and geographic information systems.',
    image: 'https://www.technogis.co.id/wp-content/uploads/2022/10/Mengenal-Theodolite-untuk-Survey-dan-Pemetaan.jpg',
    blueprint: 'https://scmohan.com.sg/wp-content/uploads/2023/05/photo-indonesias-upcoming-project-ikn-ibu-kota-negara-760x507-1.jpg',
    stats: [
      { label: 'Surveying Precision', value: 99 },
      { label: 'Cartography', value: 95 }
    ],
    notableProjects: ['The Royal Road Survey', 'The Lannisport Harbor Map']
  },
  {
    id: '2',
    name: 'Zulfikar',
    title: 'Chemical Engineer',
    specialty: 'Chemical Engineering',
    externalUrl: 'https://zulfikar-one.vercel.app/',
    bio: 'Experienced chemical engineer focused on process optimization and material refinement. Expert in developing sustainable chemical solutions for industrial applications.',
    image: 'https://www.sulzer.com/-/media/images/products/separation-technology/distillation-and-absorption/ct_103_0_production_plant_distillation_columns_1920x1280.jpg?h=1280&hash=5F9290516983FCCDC13CDB0C9AC7A52C&w=1920',
    blueprint: 'https://id.hdhce.com/upload/7065/industrial-dmac-distillation-columns-or-towers_237238.jpg',
    stats: [
      { label: 'Material Science', value: 100 },
      { label: 'Alchemy Sync', value: 92 }
    ],
    notableProjects: ['The Winterfell Fuel Cells', 'The Valyrian Steel Catalyst']
  },
  {
    id: '3',
    name: 'Fakhri',
    title: 'Industrial Legal-Technology',
    specialty: 'Industrial Control System Security',
    externalUrl: 'https://fukuru.vercel.app/',
    bio: 'Expert in industrial legal-technology and control system security, focusing on the intersection of operational efficiency and cybersecurity in complex industrial environments.',
    image: 'https://images.unsplash.com/photo-1565891741441-64926e441838?auto=format&fit=crop&q=80&w=600',
    blueprint: 'https://ik.imagekit.io/megaproject/factory/factory-5.png?tr=w-600%2Cf-auto',
    stats: [
      { label: 'Process Optimization', value: 97 },
      { label: 'Industrial Control System', value: 94 }
    ],
    notableProjects: ['The Dragonstone Supply Chain', 'The King’s Landing Assembly Line']
  },
  {
    id: '4',
    name: 'Sabian',
    title: 'Electrical Engineer',
    specialty: 'Electrical Engineering',
    externalUrl: 'https://sabian.vercel.app/',
    bio: 'Electrical engineer specializing in power distribution and circuit design. Proven track record in implementing large-scale electrical infrastructure projects.',
    image: 'https://assets.new.siemens.com/siemens/assets/api/uuid:dfbfc449-5ff4-4a05-8452-a1dc61d7453e/width:1024/quality:HIGH/dfbfc449-5ff4-4a05-8452-a1dc61d7453e-high.webp',
    blueprint: 'https://www.nextmsc.com/nextmsc-stg/images/news-transformer-monitoring-market-header-image_1756212523.jpeg',
    stats: [
      { label: 'Circuit Design', value: 95 },
      { label: 'Power Distribution', value: 100 }
    ],
    notableProjects: ['The Hightower Beacon Grid', 'The Citadel Static Storage']
  },
  {
    id: '5',
    name: 'Galeno',
    title: 'Industrial Engineer',
    specialty: 'Industrial Engineering',
    externalUrl: 'https://galeno-nine.vercel.app/',
    bio: 'Industrial systems expert specializing in workflow optimization and lean manufacturing. Dedicated to enhancing productivity through advanced system integration.',
    image: 'https://www.researchgate.net/profile/James-Boerkoel-Jr/publication/313480404/figure/fig7/AS:668654077243402@1536430963740/A-typical-automotive-assembly-line-with-a-moving-surface-Photo-Credits.ppm',
    blueprint: 'https://www.knic.co.id/uploads/6/article/automotive-factories-that-are-expanding-in-the-karawang-area-in-indonesia.jpg',
    stats: [
      { label: 'Machine Design', value: 99 },
      { label: 'Kinematics', value: 96 }
    ],
    notableProjects: ['The Storm’s End Steam Engine', 'The Iron Throne Gearbox']
  }
];
